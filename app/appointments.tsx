...
/** "today" / "tomorrow" / "in N days" — N is a whole calendar-day count. */
...
export default function Appointments() {
  ...
  const daysUntil = (iso: string) => calendarDaysUntil(new Date(iso));
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 6, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        {next && (
          <LinearGradient colors={gradient.accent} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.banner, shadow.accent]}>
            <View style={styles.bannerIcon}><Icon name="calendar" size={24} color={colors.onAccent} /></View>
            <View style={{ flex: 1, minWidth: 0 }}>
              <Text style={styles.bannerLabel}>Next up · {whenLabel(daysUntil(next.appt_at))}</Text>
      ...
      <Modal visible={showForm} animationType="slide" transparent statusBarTranslucent navigationBarTranslucent>
        <KeyboardAvoidingView style={styles.modalWrap} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={[styles.modalCard, { paddingBottom: 32 + insets.bottom }]}>
...
const makeStyles = (c: Colors) => StyleSheet.create({
  ...
  empty: { fontFamily: fonts.body5, fontSize: 14, color: c.muted, textAlign: 'center' },
  note: { fontFamily: fonts.body5, fontSize: 11, lineHeight: 16, color: c.faint, textAlign: 'center', marginTop: 22, paddingHorizontal: 14 },
  ...
  bannerTitle: { fontFamily: fonts.display, fontSize: 16, color: c.onAccent, marginTop: 6 },
  bannerWhen: { fontFamily: fonts.body5, fontSize: 11, color: 'rgba(255,255,255,0.88)', marginTop: 4 },

  card: { ...makeCardStyle(c), flexDirection: 'row', alignItems: 'flex-start', gap: 12, padding: 14, marginBottom: 10 },
  cardPast: { opacity: 0.6 },
  dateBox: { width: 50, height: 50, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  dateBoxSoft: { backgroundColor: c.accentSoft },
  dateBoxPast: { backgroundColor: c.subtleBg },
  dateDay: { fontFamily: fonts.display, fontSize: 19, color: c.accentDeep },
  dateMon: { fontFamily: fonts.body6, fontSize: 9, letterSpacing: 0.6, color: c.accent, marginTop: 2 },
  dateDayOn: { fontFamily: fonts.display, fontSize: 19, color: c.onAccent },
  dateMonOn: { fontFamily: fonts.body6, fontSize: 9, letterSpacing: 0.6, color: 'rgba(255,255,255,0.9)', marginTop: 2 },
  dateDayPast: { color: c.body },
  dateMonPast: { color: c.muted },
  cardTitle: { fontFamily: fonts.display, fontSize: 15, color: c.ink },
  cardWhen: { fontFamily: fonts.body5, fontSize: 12, color: c.accentDeep, marginTop: 4 },
  metaRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 7 },
  metaText: { fontFamily: fonts.body5, fontSize: 12, color: c.muted },
  notes: { fontFamily: fonts.body5, fontSize: 12, lineHeight: 17, color: c.muted, marginTop: 6 },
  trash: { padding: 4 },

  modalWrap: { flex: 1, backgroundColor: c.scrim, justifyContent: 'flex-end' },
  modalCard: { backgroundColor: c.canvas, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, paddingBottom: 32, maxHeight: '90%' },
  modalHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  modalTitle: { fontFamily: fonts.display, fontSize: 19, color: c.ink },
  fieldLabel: { fontFamily: fonts.body6, fontSize: 10, letterSpacing: 1.2, textTransform: 'uppercase', color: c.muted, marginBottom: 9, marginTop: 4 },
  input: { backgroundColor: c.surface, borderWidth: 1, borderColor: c.cardBorder, borderRadius: 14, padding: 14, fontFamily: fonts.body5, fontSize: 15, color: c.ink },
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 },
  chip: { paddingVertical: 8, paddingHorizontal: 13, borderRadius: 100, backgroundColor: c.accentSoft },
  chipOn: { backgroundColor: c.accentFill },
  chipText: { fontFamily: fonts.body5, fontSize: 12, color: c.accentDeep },
  chipTextOn: { color: c.onAccent },
  dateTimeRow: { flexDirection: 'row', gap: 10 },
  dtBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 9, backgroundColor: c.surface, borderWidth: 1, borderColor: c.cardBorder, borderRadius: 14, padding: 13 },
  dtText: { fontFamily: fonts.body5, fontSize: 13, color: c.ink },
  reminderPill: { flexDirection: 'row', alignItems: 'center', gap: 9, backgroundColor: c.accentSoft, borderRadius: 14, padding: 12, marginTop: 18 },
  reminderText: { fontFamily: fonts.body5, fontSize: 12, color: c.accentDeep },
});
